...
Begin VB.Form Template 
   ...
   Begin VB.Frame Frame2 
      BackColor       =   &H00C0C0C0&
      Caption         =   "Units of aperture size"
      ForeColor       =   &H00000000&
      Height          =   615
      Index           =   1
      Left            =   4800
      ...
      Top             =   480
      Width           =   2295
      Begin VB.OptionButton OptApertureSize 
         BackColor       =   &H00C0C0C0&
         Caption         =   "cm"
         Height          =   255
         Index           =   1
   ...
   Begin VB.Label LblParameter 
      BackColor       =   &H00C0C0C0&
      Caption         =   "Number of screens in paralllel"
      Height          =   255
      Index           =   7
      Left            =   120
      TabIndex        =   27
   ...
   Begin VB.Label LblParameter 
      BackColor       =   &H00C0C0C0&
      Caption         =   "Power exponent for separated region  sigma"
      Height          =   255
      Index           =   2
      Left            =   120
      TabIndex        =   16
   ...
   Begin VB.Label LblParameter 
      BackColor       =   &H00C0C0C0&
      Caption         =   "Kinetic constant for separated region s50"
      Height          =   255
      Index           =   1
      Left            =   120
      TabIndex        =   12
   ...
   Begin VB.Label LblParameter 
      BackColor       =   &H00C0C0C0&
      Caption         =   "Kinetic constant for crowded region  k0/u"
      Height          =   255
      Index           =   0
      Left            =   120
      TabIndex        =   11
...
'***************
  ...
  Caption = Caption & " ???? for unit " & CStr(UnitMods.Unit) 'Replace ???? with the 4-character model name
  ID = UnitMods.ID
  CHANGE_FLAG = 0
' Find the current data for this model if any
  If CURRMODELDATA(ID).MODEL <> "????" Then  'Replace ???? with the 4-character model name
...
Sub LoadDefault()
'*****************
  'Define the default paprmeter values here
  Parameter(0).text = CStr(1)
  Parameter(1).text = CStr(0.5)
  Parameter(2).text = CStr(2)
  Parameter(3).text = CStr(10)
  Parameter(4).text = CStr(1.2)
  Parameter(5).text = CStr(3)
  Parameter(6).text = CStr(2)
  Parameter(7).text = CStr(1)
  'Set appropriate unit options here if necessary
  OptScreenSize(0).Value = True
  OptApertureSize(0).Value = True
End Sub
...
'**************************************
  ...
  CHANGE_FLAG = 0
  NDATA.MODEL = "????" 'Replace ???? with the 4-character model name
  NDATA.NOPAR = 8      'Specify the number of parameters in your model here
  For I = 1 To NDATA.NOPAR
    NDATA.PARAM(I) = Val(Parameter(I - 1).text)
  Next I
  'Convert any parameter units if required
  'Convert the screen size to meters
  ScaleFactor = 1#
  If OptScreenSize(1).Value Then ScaleFactor = 0.3048
  NDATA.PARAM(5) = NDATA.PARAM(5) * ScaleFactor
  NDATA.PARAM(6) = NDATA.PARAM(6) * ScaleFactor
  ScaleFactor = 1
  If OptApertureSize(1).Value Then ScaleFactor = 10
  If OptApertureSize(2).Value Then ScaleFactor = 25.4
  NDATA.PARAM(4) = NDATA.PARAM(4) * ScaleFactor
End Sub
...
Sub optApertureSize_Click(Index As Integer)
'******************************************
  'Set up the change flag if an option buton is clcked
  If Screen.ActiveControl.Tag = "ApSize" Then
    If OptNew.Value = 0 Then OptNew.Value = -1
    CHANGE_FLAG = -1
  End If
End Sub

Sub optApertureSize_LostFocus(Index As Integer)
'**********************************************
  'Implement the changes as soon as the option button loses focus
  If CHANGE_FLAG = -1 Then Call MakeNew(NEWDATA)
  CHANGE_FLAG = 0
End Sub

Private Sub optApertureSize_MouseDown(Index As Integer, Button As Integer, Shift As Integer, X As Single, Y As Single)
'*********************************************************************************************************************
'Convert the units for some parameters when the option button is right clicked.
Dim I As Integer
...
If Button = vbRightButton Then
  ...
  For I = 0 To CFactor.Number - 1
    If OptApertureSize(I).Value = True Then
      Factor = CFactor.Factor(I + 1) / CFactor.Factor(Index + 1)
    End If
  Next I
  'Include all the parameters that must be changed by this option button
  Parameter(3).text = Format(Val(Parameter(3).text) * Factor, "#0.####")
  OptApertureSize(Index).Value = True
End If
...
Sub optScreenSize_LostFocus(Index As Integer)
'********************************************
  If CHANGE_FLAG = -1 Then Call MakeNew(NEWDATA)
  CHANGE_FLAG = 0
End Sub

Private Sub OptScreensize_MouseDown(Index As Integer, Button As Integer, Shift As Integer, X As Single, Y As Single)
'*****************************************************************************************************************
Dim I As Integer
Dim Factor As Double
...
If Shift = 2 And ProfessionalVersion Then
  ...
  If Index < ?? Then           '?? will usually be equal to NOPAR
