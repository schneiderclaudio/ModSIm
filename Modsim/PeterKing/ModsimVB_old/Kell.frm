...
Begin VB.Form KELL 
   ...
   Caption         =   "Specify parameters for model KELL"
   ClientHeight    =   2790
   ClientLeft      =   885
   ClientTop       =   1335
   ClientWidth     =   7800
...
On Error GoTo ErrHandler
  ...
  ID = UnitMods.ID
  CHANGE_FLAG = 0
  If NOSPLIT(UnitMods.Unit) = 3 Then
    resp = MsgBox(" Kelly's spiral model does not handle a middling stream. Choose another model for this spiral", 0)
    KELL.cmdAccept.Enabled = 0
    KELL.Parameter(0).Enabled = 0
    KELL.cmdAccept.BackColor = 7
        OptCurrent.Enabled = 0
        OptNew.Enabled = 0
        OptDefault.Enabled = 0
    Exit Sub
  End If
' Find the current data for this model if any
  If CURRMODELDATA(ID).MODEL <> "KELL" Then
