Subroutine LevMarCurveFit(JobPath,JobReadFile,NOPAR,Parameters,XLB,XUB,SumOfSquares,LowerConfLim,UpperConfLim,ConfLevel,ExitCode)
!********************************************************************************************************************************

  ! Expose subroutine LevMarCurveFit to users of this DLL
  !
  ...
  Integer*2 ExitCode
  !DEC$ ATTRIBUTES REFERENCE :: ExitCode
  ...
  Call SOFSQ(NOPAR,Parameters,SS)
	SumOfSquares(2) = SS
  If (resp1 .NE. -1  .OR.  resp2 .NE. -1) then
    ExitCode = ExitCode + 2
  End if 
