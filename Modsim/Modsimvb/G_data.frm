...
'****************************
  ...
  'Set filters
  CommonDialog1.Filter = "All files (*.*)|*.*|Lib spectrum files (*.LSF)|*.lsf"
  'Default filter
  ...
  If Mid(WSTRING, 1, 13) <> "MODSIM_LSF1.0" Then
    MsgBox "Wrong tag label in LSF file.  Expected MODSIM_LSF1.0", 16, "ERROR"
    Close #15
    GoTo ErrHandler
  End If
  NEWLDATA.NGC = Val(Sysdata.NGCM.text)
  Input #15, NGCfile
  If NGCfile <> NEWLDATA.NGC Then
    MsgBox "Number of grade classes in the file do not match number specified in system data", vbOKOnly, "ERROR"
    Close #15
    Exit Sub
  End If
  WS = 0
  'Whatever is there it will be fractional afterwards
