...
Dim SizeFactor As Single
  ...
  If Val(Me.SizeL) >= Val(Me.SizeU) Then
    MsgBox "Lower boundary of size range must be smaller than upper boundary", vbExclamation, "CAUTION"
    Me.SizeL.SetFocus
    Exit Sub
  End If
  CHANGE_FLAG = 0
...
'***********************************************************
  ...
  If Val(SizeL.text) > 0 Then
    Me.CmdNextSizeRange.Enabled = True
  Else
    Me.CmdNextSizeRange.Enabled = False
  End If
  OptSizeUnits(0) = True
  Number.text = CStr(NNDATA.NGC)
  For I = 1 To NNDATA.NGC
  ...
  Next I
  Densu(NGC - 1).text = NWDATA.LowerDensity(NGC + 1)
