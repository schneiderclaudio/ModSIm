...
'*********************
  ...
  If Trim(WSTRING) <> Trim(MODSIM.JOBNAME) Then
    WSTRING = "System data has jobname" & Trim(WSTRING) & "This could indicate a data mismatch"
    response = MsgBox(WSTRING, 1, "WARNING")
    If response = 2 Then
      Close #31
      Exit Sub
  ...
  If JOBNAME = "PSD_PLOT" Then
    ...
    For N = NumberOfDataSets + 1 To NumberOfDataSets + N_DATA_SETS
      ...
      If WSTRING = "Data set  " Then
        ...
        CURRDATA(N).ID = Val(StreamList.ListCount) + 1
        CURRDATA(N).SET_NAME = SET_NAME
        WSTRING = Str$(CURRDATA(N).ID) + " " + SET_NAME
        StreamList.AddItem WSTRING
        For M = 1 To NDC
          Input #35, CURRDATA(N).CUMSIZE(M), CURRDATA(N).Fraction(M)
          CURRDATA(N).CUMSIZE(M) = CURRDATA(N).CUMSIZE(M)
        Next M
        CURRDATA(N).NDC = NDC
        If CURRDATA(N).Fraction(1) > 1 Then
          For M = 1 To NDC
            CURRDATA(N).Fraction(M) = CURRDATA(N).Fraction(M) * 0.01
          Next M
        End If
      End If
    Next N
...
'Fill the setupdata variable
  ...
  For J = 1 To GraphList.ListCount
    ...
    For I = 1 To CURRDATA(N).NDC
      If MaxSize < CURRDATA(N).CUMSIZE(I) Then
        MaxSize = CURRDATA(N).CUMSIZE(I)
      End If
    Next I
    SetupData.GraphData(J) = CURRDATA(N)
  Next J
  SETUP_GRAPH = -1
  If ChkTruncated = vbChecked Then
...
Dim I As Integer, J As Integer, N As Integer
  ...
  For J = 1 To StreamList.ListCount
    ...
    Print #32, "Data set  ";
    Print #32, Format(CURRDATA(N).ID, "0000");
    Print #32, " ";
    Print #32, Format(CURRDATA(N).NDC, "0000");
    Print #32, " " + CURRDATA(N).SET_NAME
    For I = 1 To CURRDATA(N).NDC
      Print #32, Format$(CURRDATA(N).CUMSIZE(I), "#.0000E+#"),
      Print #32, Format$(CURRDATA(N).Fraction(I), "#.0000E+#")
    Next I
  Next J
  Close #32
End Sub
